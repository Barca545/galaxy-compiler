use super::Parser;
use crate::{
  ast::{FnSig, Func, Statement, StatementKind, Ty, P},
  interner::lookup,
  token::TokenKind,
};
...
// Refactor:
// - Replace param type error message with a real error?
...
impl Parser {
  ...
  pub fn parse_func(&mut self,) -> Statement {
    let token = self.next();
    ...
    match token.kind {
      TokenKind::FN => Statement {
        id:0,
        loc:token.loc,
        kind:StatementKind::Func(Func {
          id:0,
          loc:token.loc,
          name:self.parse_raw_ident(),
          sig:self.parse_fn_signature(),
  ...
  pub fn parse_fn_signature(&mut self,) -> FnSig {
    ...
    while let TokenKind::IDENTIFIER(name,) = self.peek().kind {
      self.next();
      self.eat_token_expect(TokenKind::COLON, "Expected a colon",).unwrap();

      if let TokenKind::TYPE(ty,) = self.next().kind {
        params.push((name, Ty::new(lookup(ty.idx,),).unwrap(),),);
      }
      else {
        panic!("Parameter \"{}\" must be a recognized type!", lookup(name.idx))
      };
...
//   pub fn parse_fn_signature(&mut self,) -> FnSig {
//     self
//       .eat_token_expect(TokenKind::LEFT_PAREN, "Syntax Error: expected
...
//       }
//       else {
//         panic!("Parameter \"{}\" must be a recognized type!",
// lookup(param.idx))       };
